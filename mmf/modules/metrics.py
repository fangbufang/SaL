...
# from packages.transformers.models.t5.tokenization_t5_fast import T5TokenizerFast
...
@registry.register_metric("sal_textvqa_accuracy")
class SaLAccuracy(BaseMetric):
    def __init__(self):
        super().__init__("sal_textvqa_accuracy")
        import mmf.utils.m4c_evaluators as evaluators
    ...
    def calculate(self, sample_list, model_output, *args, **kwargs):
        ...
        pred_answers = model_output["scores"]
        pred_answer = self.qestion_tokenizer.batch_decode(pred_answers, skip_special_tokens=True)
        ...
        for idx in range(batch_size):
            ...
            answer_words = word_tokenize(pred_answer[idx])
            answer_2 = (answer_words).replace(" 's", "'s")
            gt_answers = byte_tensor_to_object(answers[idx])
            predictions.append({"pred_answer":answer_2, "gt_answers": gt_answers})

        accuracy = self.evaluator.eval_pred_list(predictions)
        accuracy = torch.tensor(accuracy).to(pred_answers.device)

        return accuracy


@registry.register_metric("stvqa_anls")
class STVQAANLS(SaLAccuracy):
    def __init__(self):
        super().__init__()
...
class STVQAAccuracy(SaLAccuracy):
    def __init__(self):
        super().__init__()
