...
@registry.register_processor("latr_answer")
class LatrAnswerProcessor(BaseProcessor):
    """
    Process a TextVQA answer for iterative decoding in Latr
    """
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        # self.tokenizer = T5Tokenizer.from_pretrained('/data/fcy/sal/t5-large')
    ...
    def match_answer_to_vocab_ocr_seq(
        self, answer, vocab2idx_dict, max_match_num=20
    ):
        """
        ...
        answer_words = self.tokenizer(answer).input_ids
    ...
    def __call__(self, item):
        ...
        scores = torch.zeros(
            self.max_copy_steps, 32128, dtype=torch.float
        )
        ...
        if len(all_idx_seq_list) > 0:
            ...
            # !  2022.11.14 min(len(idx_seq), self.max_copy_steps) ->min(len(idx_seq)+2, self.max_copy_steps) 想让<s>的下一个词也预测为<pad> <pad> 也预测为<pad>
            dec_step_num = min(len(idx_seq), self.max_copy_steps)
            train_loss_mask[:dec_step_num] = 1.0
            # train_loss_mask[:dec_step_num-1] = 1.0

            # train_prev_inds[0] = 0
            # !  2022.11.14 与上方修改目的一致 将dec_step_num ->dec_step_num_real
            # dec_step_num_real = min(len(idx_seq), self.max_copy_steps)
            for t in range(0, dec_step_num):
                train_prev_inds[t] = idx_seq[t]
                score_idx = idx_seq[t] if t < len(idx_seq) else 1
                if t>0:
                    scores[t, score_idx] = 1.0

            # for t in range(0, dec_step_num-1):
            #     train_prev_inds[t] = idx_seq[t]
            #     score_idx = idx_seq[t] if t < len(idx_seq) else 1
            #     scores[t, score_idx] = 1.0
        else:
